...
- ライブラリで実現できる機能を自前実装してはならない。自前実装はバグの温床になる（issue #91参照）
  - demoのMIDIパース：`@tonejs/midi`等のライブラリを使うこと。`MidiReader`のような自前バイナリリーダー禁止
  - demoのオーディオ合成：Tone.jsを使うこと。`AudioContext`＋手動サイン波計算による独自合成禁止
  - demoのWAVエクスポート：既存ライブラリを使うこと。自前でRIFFヘッダーを組み立て禁止
  - 詳細は `demo/AGENTS.md` を参照すること
